...
async def _rule_velocity(db, *, buyer_id: str | None, now: datetime) -> tuple[int, list[str]]:
    if not buyer_id:
        return 0, []
...
async def _rule_self_deal_by_phone(
    db, *, buyer_id: str | None, seller_id: str | None
...
def _rule_new_account_high_value(
    *, buyer: dict[str, Any] | None, gross: float, now: datetime
) -> tuple[int, list[str]]:
...
async def _rule_refund_rate(db, *, seller_id: str | None) -> tuple[int, list[str]]:
    if not seller_id:
        return 0, []
...
async def _rule_watchlist(db, *, user_id: str | None, party: str) -> tuple[int, list[str]]:
...
async def mark_reviewed(db, *, signal_id: str, reviewer_id: str, action: str, note: str = "") -> dict[str, Any]:
    ...
    if action == "block_order" and sig.get("order_id"):
        await db.orders.update_one(
            ...
            {"$set": {"status": "cancelled", "cancelled_reason": "fraud_review", "cancelled_at": datetime.now(UTC)}},
        )
    return sig
