...
# ───────────────────────────────────────────────────────────────────────────
# Static-ish files generated by scripts/ (pitch deck, technical doc).
# Served via the API router so the same auth/CORS chain applies and we can
# audit downloads later if needed.
# ═══════════════════════════════════════════════════════════════════════════

@api_router.get("/docs/seller-pitch.pptx")
async def download_seller_pitch():
    """
    Kiswahili-primary seller pitch deck — 11 slides explaining how Biz-Salama
    works and the benefits for sellers. Regenerate via:
        cd /app/backend && python3 scripts/generate_seller_pitch.py
    """
    from fastapi.responses import FileResponse
    path = "/app/biz_salama_seller_pitch_sw.pptx"
    if not os.path.exists(path):
        raise HTTPException(status_code=404, detail="Pitch deck not generated yet")
    return FileResponse(
        path,
        media_type="application/vnd.openxmlformats-officedocument.presentationml.presentation",
        filename="Biz-Salama-Mwongozo-Wa-Muuzaji.pptx",
